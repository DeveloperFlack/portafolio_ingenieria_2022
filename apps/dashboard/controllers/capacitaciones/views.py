    ...
    if request.method == "POST":
        ...
        if (v_idCapacitacion == ""):
            ...
            if (exist != ()):
                ...
                fc_update_capacitacion(v_nombre_capacitacion, v_descripcion_capacitacion, v_total_capacitacion)
    ...
    for i in data_to_array:
        i['usuarios'] = """
            <div class='text-center'>
                <button class="btn btn-sm btn-primary form-control" name="txtRutUsuario" id="txtRutUsuario" style="width: fit-content;">Seleccionar Profesional</button>
            </div>
        """
    # Añadir HTML
    for i in data_to_array:
...
# UPDATE CAPACITACIÓN
def dashboard_update_capacitacion(request):
    if (request.method) == 'POST':
        v_id_capacitacion = request.POST.get("idCapacitacion")
        ...
        if (exist != ()):
            ...
            fc_update_capacitacion(0, v_nombre_capacitacion, v_descripcion_capacitacion, v_total_capacitacion)
            messages.add_message(request, messages.SUCCESS, 'Capacitación actualizada!')
